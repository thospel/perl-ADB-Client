...
# Check zero add
$args = [{ in => "ZZ" }, 0, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "ZZ"']);
is($args->[0]{in}, "ZZ");

$args = [{ in => "OKAY" }, 0, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated prefix "OKAY"']);
is($args->[0]{in}, "OKAY");

$args = [{ in => "FAIL" }, 0, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated prefix "FAIL"']);
is($args->[0]{in}, "FAIL");

# A previous call will already have recognized this as Bad instead of truncated
$args = [{ in => "OKAZ" }, 0, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated prefix "OKAZ"']);
is($args->[0]{in}, "OKAZ");

# Actually check that
$args = [{ in => "OKAZ" }, 1, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "OKAZ"']);
is($args->[0]{in}, "OKAZ");

$args = [{ in => "O" }, 0, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated prefix "O"']);
is($args->[0]{in}, "O");

$args = [{ in => "FAI" }, 0, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated prefix "FAI"']);
is($args->[0]{in}, "FAI");

$args = [{ in => "OKAYZ" }, 0, 2];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated response "OKAYZ"']);
is($args->[0]{in}, "OKAYZ");

# FAIL converts $nr=2 to $nr=-1
$args = [{ in => "FAILZ" }, 0, 2];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad hex length "Z"']);
is($args->[0]{in}, "FAILZ");
...
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Spurious bytes in response "OKAYZZZZ"']);
is($args->[0]{in}, "OKAYZZZZ");

# Add too few bytes to OKAY and do EOF
$args = [{ in => "FAIL0003ZZ" }, 0, 3];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Truncated answer (expected 3, got 2 bytes)']);
...
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Spurious bytes in response "FAIL0003ZZZZ"']);
...
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad hex length "ZZZZ"']);
...
$args = [{ in => "K" }, 1, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "K"']);
is($args->[0]{in}, "K");
...
$args = [{ in => "OKK" }, 3, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "OKK"']);
is($args->[0]{in}, "OKK");
...
$args = [{ in => "OKAQ" }, 4, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "OKAQ"']);
is($args->[0]{in}, "OKAQ");
...
$args = [{ in => "FK" }, 1, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "FK"']);
is($args->[0]{in}, "FK");
...
$args = [{ in => "FAIK" }, 1, -1];
is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "FAIK"']);
is($args->[0]{in}, "FAIK");
...
for my $add (1..3) {
    ...
    $args = [{ in => "FAF" }, $add, 0];
    is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "FAF"']);
    is($args->[0]{in}, "FAF");
}
...
for my $add (1..4) {
    ...
    $args = [{ in => "FAIF" }, $add, 0];
    is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "FAIF"']);
    is($args->[0]{in}, "FAIF");
}
...
for my $add (1..5) {
    ...
    if ($add == 1) {
        ...
        is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Spurious bytes in response "OKAY0"']);
    }
    is($args->[0]{in}, "OKAY0");
...
for my $add (1..5) {
    $args = [{ in => "OKAYQ" }, $add, -1];
    is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad hex length "Q"']);
    is($args->[0]{in}, "OKAYQ");
}

for my $add (1..8) {
    $args = [{ in => "OKAY000Q" }, $add, -1];
    is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad hex length "000Q"']);
    is($args->[0]{in}, "OKAY000Q");
}

for my $add (1..4) {
    # Bad prefix is ignored since it should have been detected earlier
...
for my $add (1..9) {
    ...
    if ($add == 1) {
        ...
        is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Spurious bytes in response "OKAY0000Z"']);
    }
    ...
    if ($add == 1) {
        ...
        is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Spurious bytes in response "FAIL0000Z"']);
    }
    ...
    if ($add <= 5) {
        if ($add == 1) {
            ...
            # Bad prefix should have been detected earlier
            is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Spurious bytes in response "ZZZZ0000Z"']);
            is($args->[0]{in}, "ZZZZ0000Z");
        }
    } else {
        is_deeply([adb_check_response(@$args)], [BAD_ADB, 'Bad prefix "ZZZZ"']);
...
for my $add (1..2) {
    ...
    if ($add == 1) {
        ...
        is_deeply([adb_check_response(@$args)],
                  [BAD_ADB, 'Spurious bytes in response "OKAK0001ZZ"']);
    }
